...
%\added{Successful implementation of the SMS techniques described here requires mode identification to determine whether the observed MHD mode is surface or body. Surface modes have been indirectly identified in photospheric pores \citep{mor_etal15}. Higher spatial resolution will allow us to more consistently and accurately determine whether the oscillation is maximum at the boundaries of or within the waveguide, to differentiate between surface and body MHD modes, respectively. This will be crucial for the implementation of the SMS techniques described here, which can only apply to surface modes.}

\added{Large magnetic bright points (MBPs) in large diameter waveguides ($L > 500km$) along inter-granular lanes are often elongated \citep{cro_etal10}. The application of these SMS techniques to MBPs is limited by the low spatial resolution of current observations. DKIST is proposed to have a spatial resolution of 19km for structures on the solar surface \citep{tri_etal15}, sufficient to resolve oscillations in MBPs. The resolution will give a sufficient number of pixels (5-10) across an MBP to determine whether the oscillation is maximum at the boundaries of or within the waveguide, to differentiate between surface and body MHD modes, respectively. This is crucial for the accurate employment of these magneto-seismology techniques and would build upon previous work on mode identification such as the surface modes that were identified in photospheric pores \citep{mor_etal15}.}

\added{Light walls also present a potential application for these magneto-seismology techniques. Observed as groups of surges and other magnetic structures rooted above sunspot light bridges, light walls have been demonstrated to guide MHD waves driven by a nearby disturbance \citep{yan_etal15}. The low solar atmosphere above a sunspot umbra (the area around the light bridge) has a strong magnetic field, which puts into question the application of the current model of a magnetic slab embedded in a non-magnetic plasma. However, the sound speed in 

SURELY THE SOUND SPEED IS MUCH LOWER THAN THE ALFVEN SPEED SO EVEN KINK MODE ISNT INCOMP!?

applicable with incompressible approximation (just considering kink modes). Or more accurately we could apply a generalised version of these methods which involve an external magnetic field. Could be highly non-linear (zha_etal17).}

We propose to determine whether asymmetric magneto-acoustic surface waves can be excited within the characteristic lifetime of an asymmetric waveguide in the solar atmosphere. This task can be investigated analytically (for linear waves with simple initial conditions) and numerically (for non-linear waves with more sophisticated initial conditions). Further, a more realistic system, including asymmetric magnetic fields in the external plasmas, or an equilibrium shear flow, would allow for better application to solar waveguides.
